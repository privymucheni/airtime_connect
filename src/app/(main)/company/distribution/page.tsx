...
const CompanyDistribution: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Get raw rows to find headers robustly
        ...
        // Find the header row by scanning first 10 rows
        ...
        if (headerRowIndex === -1) {
          throw new Error("File must contain columns for 'Phone Number' (or 'MSISDN') and 'Amount'. Please ensure these headers are in one of the first 10 rows.");
        }
        ...
        for (let i = headerRowIndex + 1; i < rawRows.length; i++) {
          ...
          if (!phoneNumberRaw || isNaN(amount) || amount <= 0) {
            // Only count as skipped if it's not a completely empty row
            if (phoneNumberRaw || amountRaw) {
              skippedCount++;
            }
            continue;
          }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Bulk <span className="text-indigo-600">Distribution</span></h2>
        <p className="text-lg text-gray-500 font-medium">Upload your spreadsheet to credit multiple numbers instantly.</p>
      </div>
          ...
          {!recipients.length ? (
            ...
            >
              ...
              <p className="text-sm text-gray-400 font-medium max-w-sm mx-auto mt-2">
                Supported: CSV, XLS, XLSX. Ensure your file contains 'Phone Number' and 'Amount' columns.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center justify-between bg-indigo-50 p-5 rounded-2xl">
                <div className="flex items-center space-x-3 text-indigo-700">
                  ...
                  <div>
                    <p className="font-black text-lg text-indigo-900">{recipients.length} Recipients Validated</p>
                    <p className="text-sm font-bold text-indigo-600/80 uppercase tracking-wider">
              ...
              <div className="overflow-hidden border border-gray-100 rounded-2xl shadow-sm">
                <table className="w-full text-left">
                  <thead className="bg-gray-50/50 border-b border-gray-100">
                    <tr>
                      <th className="px-8 py-4 text-sm font-black text-gray-400 uppercase tracking-widest">Employee Name</th>
                      <th className="px-8 py-4 text-sm font-black text-gray-400 uppercase tracking-widest">Phone Number</th>
                      <th className="px-8 py-4 text-sm font-black text-gray-400 uppercase tracking-widest text-right">Credit Amount</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {recipients.map((r, i) => (
                      <tr key={i} className="hover:bg-gray-50/30 transition-colors">
                        <td className="px-8 py-4 text-xl font-black text-gray-900">{r.name}</td>
                        <td className="px-8 py-4 text-base text-gray-500 font-bold">{r.phoneNumber}</td>
                        <td className="px-8 py-4 text-right font-black text-indigo-600 text-xl tracking-tight">${r.amount.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-slate-900 p-6 rounded-2xl flex items-center justify-between text-white">
                <div>
                  <p className="text-gray-400 text-[10px] uppercase font-medium tracking-[0.2em] mb-1">Grand Total Cost</p>
                  <p className="text-3xl font-black tracking-tight text-white">${totalToDistribute.toLocaleString()}</p>
                  {!hasSufficientBalance && (
                    <p className="text-red-400 text-xs mt-2 font-bold bg-red-400/10 px-3 py-1.5 rounded-xl inline-block uppercase tracking-wider">Insufficient balance</p>
                  )}
                </div>
                <button
                  onClick={handleExecute}
                  disabled={isExecuting || !hasSufficientBalance}
                  className={`px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-sm rounded-2xl shadow-xl shadow-indigo-900/40 transition-all flex items-center space-x-2 transform ${isExecuting || !hasSufficientBalance ? 'opacity-50 cursor-not-allowed' : 'hover:-translate-y-1 active:scale-95'}`}
                >
                  {isExecuting ? (
                  ...
                  <span className="uppercase tracking-widest">{isExecuting ? 'Processing...' : 'Execute'}</span>
                </button>
              </div>
            </div>
          )}
